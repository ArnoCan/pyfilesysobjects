...
class CallUnits(unittest.TestCase):
    ...
    def testCase010(self):
        ...
        resX = [
            os.path.normpath(tdata + '/A.txt'),
            os.path.normpath(tdata + '/a.sh'),
            os.path.normpath(tdata + '/a.smod')
        ]
    ...
    def testCase011(self):
        ...
        arg = filesysobjects.apppaths.normapppathx(arg)
        res = filesysobjects.pathtools.expandpath(arg)
        resX = [os.path.normpath(tdata + '/b/libb.sh')]
        self.assertEqual(sorted(res), sorted(resX))


    def testCase020(self):
        arg = tdata + '/b/"""[!x]*/"""[c][.][p][lm]'
        arg = filesysobjects.apppaths.normapppathx(arg)
        res = filesysobjects.pathtools.expandpath(arg)
        resX = [os.path.normpath(tdata + '/b/c/c.pl'), os.path.normpath(tdata + '/b/c/c.pm')]
        self.assertEqual(sorted(res), sorted(resX))
