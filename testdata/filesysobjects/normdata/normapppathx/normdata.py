...
class CallUnitsBase(unittest.TestCase):
    ...
    * erridx: current error index
    ...
    def __str__(self):
        ...
        res += ("# GOT:     result[%3d]    = '%s'")%(self.curidx,str(normapppathx(self.norm_data[self.curidx],tpf=self.curtpf,strip=self._strip)))
        return res
    ...
    def printExpect(self):
        ...
        for i in range(len(self.norm_data)):
            if i%5 == 0:
                print ("#--- %d")%(i)
            print ("\tr'%s', # %d")%(str(normapppathx(self.norm_data[i],tpf=self.curtpf,strip=self._strip)),i)
        print("]")
        pass
...
#         return len(self.norm_data)
    ...
    def calltests(self,**kargs):
        ...
        if len(self.norm_data) != len(self.norm_result):
            ...
            sys.stderr.write("# DATA:       "+str(self.norm_data_source)+" : "+str(13)+"\n")
            ...
            self.assertEqual(
                ...
                "Sets mismatch: self.norm_data="+str(len(self.norm_data))+" norm_result="+str(len(self.norm_result))
                )
        ...
        for i in range(f,l):
            ...
            try:
                ...
                res = normapppathx(self.norm_data[i], **kwargs)
                ...
                sys.stderr.write(("  <-  GOT:       normapppathx()[%3d] = '%s'\n")%(i,str(res)))
